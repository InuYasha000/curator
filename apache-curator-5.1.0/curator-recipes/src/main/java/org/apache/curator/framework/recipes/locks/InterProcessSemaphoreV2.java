...
public class InterProcessSemaphoreV2
{
     ...
     * <code>finally</code> block. NOTE: You can use {@link #returnAll(Collection)} for this.</p>
     *
     * @param qty  number of leases to acquire   1
     * @param time time to wait  0
     * @param unit time unit  null
    ...
    private InternalAcquireResult internalAcquire1Lease(ImmutableList.Builder<Lease> builder, long startMs, boolean hasWait, long waitMs) throws Exception
    {
        ...
        try
        {
            ...
            //这个Lease对象其实就代表了客户端A成功加的一个semaphore的这个锁
            //在这个里面重载了lease的close方法，删除leases目录下的临时顺序节点删了
            ...
            try
            {
                synchronized(this)
                {
                    for(;;)
                    {
                        ...
                        try
                        {
                            //在这里children就是 [_c_a9302e20-de9c-4356-923a-274664d7676c-lease-0000000000]
                            // 当前/semaphores/semaphore_01/leases 目录下的节点的数量是否<=3
                            //在这里对 leasesPath 添加了监听器，当 leases 下面的锁释放后（也就是删除了leases下面的目录）就会调用这个监听器的 processon 方法
